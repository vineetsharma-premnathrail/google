...
const CATEGORY_GRADIENT: Record<string, string> = {
  draft: "from-slate-800 to-slate-900",
  confirmed: "from-blue-950 to-slate-900",
  active: "from-emerald-950 to-slate-900",
  completed: "from-slate-800 to-slate-900",
};
...
export default function TripCard({ trip, onClick }: { trip: Trip; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`card text-left w-full hover:border-brand-600 hover:shadow-lg hover:shadow-brand-900/20 transition-all bg-gradient-to-br ${CATEGORY_GRADIENT[trip.status]}`}
    >
      {/* Header */}
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div className="flex items-center gap-1.5 text-slate-300">
          ...
          <span>{format(new Date(trip.start_date), "MMM d")} · {nights}n</span>
        </div>
        <div className="flex items-center gap-1.5 text-slate-300">
        ...
        {trip.budget_total && (
          <div className="flex items-center gap-1.5 text-slate-300">
            ...
            <span>${trip.budget_total.toLocaleString()} {trip.budget_currency}</span>
          </div>
        )}
