...
class AIPlanner:
    ...
    async def refine_with_chat(
        ...
        """Conversational refinement of an existing itinerary."""
        messages = conversation_history.copy()
        messages.append({
            "role": "user",
            "content": f"""Current itinerary: {json.dumps(current_itinerary, indent=2)}
...
Apply the user's requested changes and return the full updated itinerary in the same JSON schema."""
    ...
    async def handle_disruption(
        ...
        """Re-plan affected days due to a real-world disruption."""
